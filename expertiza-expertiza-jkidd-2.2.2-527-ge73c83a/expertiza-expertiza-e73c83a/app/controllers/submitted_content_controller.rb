...
class SubmittedContentController < ApplicationController
  ...
  def folder_action
    ...
    if params[:faction][:delete]
      ...
      move_selected_file
    elsif params[:faction][:copy_questionnaire]
      copy_selected_file
...
private  
  ...
  def copy_selected_file
    old_filename = params[:directories][params[:chk_files]] +"/"+ params[:filenames][params[:chk_files]]
    new_filename = params[:directories][params[:chk_files]] +"/"+ FileHelper::sanitize_filename(params[:faction][:copy_questionnaire])
    begin   
      if File.exist?(new_filename)
