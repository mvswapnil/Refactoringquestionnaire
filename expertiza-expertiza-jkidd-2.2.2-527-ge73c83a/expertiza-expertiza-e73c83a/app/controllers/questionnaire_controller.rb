class QuestionnaireController < ApplicationController
  ...
  def add_new_advice(question_id,new_question_id)
    advice = QuestionAdvice.find_by_question_id(question_id)
    if !(advice.nil?)
      new_advice = advice.clone
      new_advice.question_id = new_question_id
      new_advice.save
    end
  end

  def add_advice_for_new_question(questionnaire_id)
    ...
    questions.each{ | question |
      ...
      add_new_advice(question.id,new_question.id)
      ...
      if (@questionnaire.section == "Custom")
        ...
        if !(old_question_type.nil?)
          ...
          new_question_type.question_id = new_question.id
  ...
  def copy_questionnaire
    ...
    begin
      ...
      add_advice_for_new_question(@questionnaire.id)
      parent_folder = TreeFolder.find_by_name(@questionnaire.display_type)
      parent = FolderNode.find_by_node_object_id(parent_folder.id)
    ...
    if @questionnaire
       begin
          ...
          for question in @questionnaire.questions
            current_question_type = QuestionType.find_by_question_id(question.id)
            if !current_question_type.nil?
             current_question_type.delete
            end
          end
  ...
  def save_new_question_parameters(question_id, question_number)
    question = QuestionType.new
    question.q_type = params[:question_type][question_number][:type]
    question.parameters = params[:question_type][question_number][:parameters]
  ...
  def save_new_questions(questionnaire_id)
    ...
    if params[:new_question]
      ...
      for question_key in params[:new_question].keys
        question = Question.new(params[:new_question][question_key])
        question.questionnaire_id = questionnaire_id
        if question.true_false == ''
          question.true_false = false
        end
        if !question.txt.strip.empty?
          question.save
          ...
          if (questionnaire.section == "Custom")
            for i in (questionnaire.min_question_score .. questionnaire.max_question_score)
              advice = QuestionAdvice.new(:score => i, :advice => nil)
              advice.question_id = question.id
              advice.save
            end
            save_new_question_parameters(question.id, question_key)
          end
